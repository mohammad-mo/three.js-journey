...
void main()
{
    ...
    // Or --> float strength = ceil(vUv.x * 10.0) / 10.0;
    ...
    // Or
    // float strength = 0.15 / (distance(vec2(vUv.x, (vUv.y - 0.5) * 5.0 + 5.0), vec2(0.5)));
    ...
    // step returning a value which can be 0 or 1, with 1 - step, we can return the opposite value
